...
- **コード → コード** — AST から解決したインポート（`TYPE_CHECKING` 対応）。
  Python は標準で同梱。リゾルバはパッケージ内で唯一の言語依存部分です —
  [別の言語](#別の言語)を参照
- **コード ↔ ドキュメント** — どの markdown ファイルがどのソースを言及している
  か。突き合わせは決定的で、途中にモデルを挟みません。同じ入力からは常に同じ
  答えが出るので、CI のゲートに使えます
- **git ドリフト** — added / modified / renamed / deleted のオーバーレイと、
  もう存在しないファイルのゴーストノード、あるいは任意の 2 つの ref 間の差分
...
**すべて手元のマシンで完結します。** API キーもアカウントもテレメトリも不要。
コードもドキュメントもリポジトリの外へ出ません。解析は素のパースであって LLM の
パスではないため、実行コストはゼロで、毎回まったく同じグラフになります。
エクスポートは、すでに料金を払っているエージェントに渡してください。

**依存ゼロ** — これは現状ではなく、譲れない制約です。純粋な Python 標準
ライブラリのみで、`pip install` は他に何も引き込みません。コンパイルすべき
文法も、データベースも、バックグラウンドのインデクサも、デーモンもありません。
唯一のサードパーティコードはブラウザ上の D3.js で、SRI ピン留めした CDN から
読み込むか、`--no-cdn` で完全に埋め込めます。埋め込めば HTML は本当に自己完結し、
このツールを一度も入れたことのないマシンでもオフラインで開けます。
...
- リンクはファイル全体に解決されます — セクションアンカー（`file.md#part`）は
  ファイルノードにマップされます。
- code→code エッジは今のところ Python のみです（他の層はすべて言語非依存 —
  下記参照）。
- グラフ 1 つにつきリポジトリ 1 つ。シンボリックリンクは物理パスとして扱われます。

## 別の言語

言語を知っているのはインポートリゾルバだけです。パッケージ約 5,600 行のうち
およそ 450 行、しかも 1 つのモジュールに収まっています。他言語のファイルは
**すでに**ノードになり、ドキュメント・git・ヒート・カバレッジの各層もすでに
持っています。新しい言語が加えるのは、ソース同士をつなぐエッジだけです。

残りはそのまま再利用できます。git オーバーレイと ref 差分は git を呼び、
ヒートマップはコミット数を数え、カバレッジ層は Cobertura XML を読み（JaCoCo、
istanbul/nyc、coverlet、gocover-cobertura がいずれも出力します）、
ドキュメント層は markdown を走査します。HTML フロントエンド、JSON エクスポート、
`graph-query`、`find-related-docs`、`verify-doc-links` はソースの構文を一切
見ません。

つまり Go や TypeScript への移植は、文書化された契約に沿ってリゾルバを 1 つ
書くことであって、ツールを作り直すことではありません。契約、差し込み位置、
エッジの形は
[CONTRIBUTING.md](../CONTRIBUTING.md#adding-another-language) にあります。
stdlib のみ: コンパイルする文法も、ネイティブ wheel も、`pip install` を
ビルドに変えるものも一切ありません。
