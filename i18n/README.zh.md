...
- **代码 → 代码** — 从 AST 解析出的导入（感知 `TYPE_CHECKING`）。Python 开箱
  即用；解析器是整个包里唯一与语言绑定的部分 —— 见[另一种语言](#另一种语言)
- **代码 ↔ 文档** — 哪些 markdown 文件提到了哪些源文件；匹配是确定性的，回路里
  没有模型: 相同输入永远给出相同答案，因此可以作为 CI 门禁
- **git 漂移** — added / modified / renamed / deleted 的叠加层，外加为已不存在
  的文件保留的幽灵节点，或任意两个 ref 之间的 diff
...
**全部在你自己的机器上运行。** 不需要 API key、不需要账号、没有遥测；代码和
文档都不会离开仓库 —— 分析就是普通解析，而不是一次 LLM 调用，因此运行不花钱，
而且每次都给出同一张图。导出的文件交给你已经在付费的那个智能体即可。

**零依赖**，而且这是硬性约束，不是当前状态: 纯 Python 标准库，`pip install`
不会额外拉取任何东西。没有需要编译的语法、没有数据库、没有后台索引器、没有
守护进程。唯一的第三方代码是浏览器中的 D3.js，通过带 SRI 固定的 CDN 加载，或用
`--no-cdn` 完全内嵌 —— 内嵌之后这份 HTML 真正自成一体，在从未装过本工具的机器上
也能离线打开。
...
- code→code 边目前仅限 Python（其余各层均与语言无关 —— 见下文）。
- 一个仓库对应一张图谱；符号链接按物理路径处理。

## 另一种语言

只有导入解析器知道自己在读哪种语言 —— 在整个包约 5,600 行里约占 450 行，且全部
集中在一个模块。其他语言的文件**已经**会成为节点，也已经带上各自的文档、git、
热度与覆盖率图层；新增一种语言，补的只是源文件之间的边。

其余部分原样复用: git 叠加层与 ref diff 调用 git，热力图统计提交次数，覆盖率
图层读取 Cobertura XML（JaCoCo、istanbul/nyc、coverlet 和 gocover-cobertura
都会产出这种格式），文档层扫描 markdown；而 HTML 前端、各种 JSON 导出、
`graph-query`、`find-related-docs` 和 `verify-doc-links` 根本不看源码语法。

所以移植到 Go 或 TypeScript，是照着一份写明的契约写一个解析器，而不是重造工具。
契约、接入点和边的形状都在
[CONTRIBUTING.md](../CONTRIBUTING.md#adding-another-language)。只用 stdlib:
没有要编译的语法，没有原生 wheel，也没有任何会把 `pip install` 变成一次构建的
东西。
