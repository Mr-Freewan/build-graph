...
Агенту отдают ультракомпактный снимок: тот же граф, что и в остальных экспортах,
примерно за треть токенов, и единственный, который несёт ещё слои Heat и
Coverage:

```bash
build-graph --ultra-compact                            # docs/graph-ultra.json
build-graph --ultra-compact --coverage coverage.xml    # + колонка покрытия
build-graph --bench                                    # во что обходится каждый экспорт
```

Ключ `legend` объясняет файл тому, кто его читает, так что схема рядом не нужна;
`graph-query` тоже его читает и берёт по умолчанию.
