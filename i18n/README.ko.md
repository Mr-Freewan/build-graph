...
- **코드 → 코드** — AST 에서 해석한 임포트(`TYPE_CHECKING` 인식). Python 은
  기본 제공이며, 리졸버는 패키지에서 유일하게 언어에 묶인 부분입니다 —
  [다른 언어](#다른-언어) 참고
- **코드 ↔ 문서** — 어떤 markdown 파일이 어떤 소스를 언급하는지. 대응은
  결정적이며 중간에 모델이 없습니다. 같은 입력이면 언제나 같은 답이 나오므로
  CI 게이트로 쓸 수 있습니다
- **git 드리프트** — added / modified / renamed / deleted 오버레이와, 더 이상
  존재하지 않는 파일을 위한 고스트 노드, 또는 임의의 두 ref 사이의 diff
...
**전부 사용자 컴퓨터에서 돌아갑니다.** API 키도, 계정도, 텔레메트리도 없고,
코드와 문서 어느 쪽도 저장소를 벗어나지 않습니다. 분석은 LLM 패스가 아니라 순수
파싱이라 실행 비용이 없고 매번 같은 그래프가 나옵니다. 내보낸 파일은 이미
비용을 내고 쓰는 에이전트에게 주면 됩니다.

**의존성 제로** 이며, 이는 현재 상태가 아니라 지켜야 할 제약입니다. 순수 Python
표준 라이브러리만 쓰며, `pip install` 은 그 외 아무것도 끌어오지 않습니다.
컴파일할 문법도, 데이터베이스도, 백그라운드 인덱서도, 데몬도 없습니다. 유일한
서드파티 코드는 브라우저의 D3.js 로, SRI 고정과 함께 CDN 에서 불러오거나
`--no-cdn` 으로 완전히 임베드할 수 있습니다. 임베드하면 HTML 은 진짜로 자기
완결적이어서, 이 도구를 한 번도 설치한 적 없는 컴퓨터에서도 오프라인으로
열립니다.
...
- 링크는 파일 전체로 해석됩니다 — 섹션 앵커(`file.md#part`)는 파일 노드로
  매핑됩니다.
- code→code 엣지는 현재 Python 전용입니다(나머지 계층은 모두 언어 독립적 —
  아래 참고).
- 그래프 하나당 저장소 하나; 심링크는 물리 경로로 취급됩니다.

## 다른 언어

언어를 아는 것은 임포트 리졸버뿐입니다. 패키지 약 5,600 줄 가운데 450 줄 남짓이며
모두 한 모듈에 있습니다. 다른 언어의 파일도 **이미** 노드가 되고 문서·git·히트·
커버리지 계층을 이미 갖고 있습니다. 새 언어가 더하는 것은 소스 사이의 엣지뿐입니다.

나머지는 그대로 재사용됩니다. git 오버레이와 ref diff 는 git 을 호출하고,
히트맵은 커밋 수를 세고, 커버리지 계층은 Cobertura XML 을 읽으며(JaCoCo,
istanbul/nyc, coverlet, gocover-cobertura 가 모두 이 형식을 냅니다), 문서 계층은
markdown 을 훑습니다. HTML 프런트엔드, JSON 내보내기, `graph-query`,
`find-related-docs`, `verify-doc-links` 는 소스 문법을 전혀 보지 않습니다.

따라서 Go 나 TypeScript 로 옮기는 일은 문서화된 계약에 맞춰 리졸버 하나를 쓰는
것이지 도구를 다시 만드는 것이 아닙니다. 계약과 연결 지점, 엣지 형태는
[CONTRIBUTING.md](../CONTRIBUTING.md#adding-another-language) 에 있습니다.
stdlib 만 사용합니다: 컴파일할 문법도, 네이티브 wheel 도, `pip install` 을 빌드로
바꾸는 것도 없습니다.
